...
import type { AccountMetadata } from "../../../core/schemas.js";
 ...
 * never fire multiple refresh grants (which could each rotate the refresh
 ...
 *
 * HARD CONTRACT: refresh may rotate the refresh token on every grant, so there
 * is at most one live refresh token at a time. Callers MUST treat the returned
 ...
 * token yields invalid_grant and would wrongly mark the account dead. The pool
 * must pass (and update) the single canonical account entry, never a detached
 * snapshot.
 *
...
export async function getFreshTokens(
  ...
  const run = (async (): Promise<Tokens> => {
    ...
    // Server may have rotated the refresh token: `tokens` holds the live one.
    // Persist (atomically) with retry BEFORE returning.
